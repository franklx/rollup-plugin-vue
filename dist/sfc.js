...
function transformSFCEntry(
  ...
  if (errors.length) {
    errors.forEach((error) =>
      pluginContext.error(error_1.createRollupError(filename, error))
    )
    return null
  }
  const shortFilePath = path_1.default
    ...
    .replace(/^(\.\.[\/\\])+/, '')
    .replace(/\\/g, '/')
  const scopeId = hash_sum_1.default(
  ...
  if (!isProduction) {
    ...
    output.push(
      `script.__file = ${JSON.stringify(
        path_1.default.basename(shortFilePath)
      )}`
    )
  }
...
function genScriptCode(
  ...
  const script = script_1.resolveScript(
    descriptor,
    scopeId,
    isProd,
    isServer,
